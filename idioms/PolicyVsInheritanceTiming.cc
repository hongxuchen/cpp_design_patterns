...
class DoOperationPolicy : public operation_policy {
 ...
 public:
  float Run(const float a, const float b) { return Operation(a, b); }
};

class OperationPolicy_Add {
 protected:
  float Operation(const float a, const float b) { return a + b; }
};
...
class OperationAdd : public DoOperation {
 public:
  float Run(const float a, const float b) { return a + b; }
};
...
int main() {
  // In debug mode, policy takes .13 seconds, while polymorphic takes only .05
  // seconds. (1e7 iterations)
  boost::timer timer;
